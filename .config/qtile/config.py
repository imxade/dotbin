...
keys = [
    ...
    Key(
        ["mod1"], "s", lazy.spawn(f"sh {SCRIPTS_DIR}/screenshot.sh"), desc="screenshot"
    ),
    Key(
        ["mod1"],
        "v",
        lazy.spawn(f"sh {SCRIPTS_DIR}/screencast.sh"),
        desc="start screencap",
    ),
    Key(
        ["mod1"],
        "k",
        lazy.spawn(f"sh {SCRIPTS_DIR}/killcast.sh"),
        desc="stop screencap",
    ),
    Key(
        [mod],
        "a",
        lazy.spawn(f"cp -sf ~/.config/alsa/bluetooth ~/.asoundrc"),
        desc="switch to bluetooth device",
    ),
    Key(
        [mod, "shift"],
        "a",
        lazy.spawn(f"cp -sf ~/.config/alsa/speaker ~/.asoundrc"),
        desc="switch to speaker",
    ),
...
def autostart_once():
    subprocess.run(
        f"{HOME}/.config/qtile/autostart.sh"
